...
def load_config(config_path: str) -> dict[str, dict[str, str]]:
    ...
    try:
        ...
        raise ValueError(
            f"Couldn't load configuration file located at {config_path}. {e}"
        )


def load_account_attributes(config_path: str) -> dict[str, dict[str, str]]:
    """Opens an account configuration file and returns the result as a python dict object.
...
def get_out_file_path(base_dir: str, path: str) -> str:
    ...
    if path == "data/out/transactions.csv":
        ...
        if is_valid_csv_file_path(path):
            ...
            raise ValueError(
                f"The filepath {path} is not valid. Either the directory doesn't exist or the file is not of type CSV."
            )


def is_valid_csv_file_path(file_path: str) -> bool:
    """
...
def enforce_dataframe_schema(df, schema):
    ...
    if missing_columns:
        raise ValueError(
            f"Transactions dataframe has missing columns. Missing columns: {missing_columns}"
        )
    ...
    if extra_columns:
        raise ValueError(
            f"Transactions dataframe has extra columns. Extra columns: {extra_columns}"
        )
    ...
    for col, dtype in schema.items():
        if col in df.columns:
            ...
            current_dtype_str = str(df[col].dtype)
            if dtype == "datetime64[ns]" and current_dtype_str.startswith("datetime64"):
                pass
            elif current_dtype_str != dtype:
                raise ValueError(
